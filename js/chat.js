...
var chat = new function(){
	...
	function getChatHistory(callback){
		...
		var chats = [];
		chats.push({datetime:new Date(d.setTime(d.getTime()+20000000)).toISOString(), message:"hello", from:"Visitor"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+40000000)).toISOString(), message:"Hi, how can I help you?", from:"Operator"});
		chats.push({datetime:new Date(d.setTime(d.getTime()+40000000)).toISOString(), message:"I'm looking for a size 7, but can't find it", from:"Visitor"});
		...
		chats.push({datetime:new Date(d.setTime(d.getTime()+20000000)).toISOString(), message:"I'm sorry, there is no sie 7 available in that colour. There are some in red and blue however", from:"Operator"})
	...
	function dispatchChatEvent(msg, from){
		...
		var newEvent = event.detail;
		dynamic.dynamicChat(newEvent, "liveChat");
